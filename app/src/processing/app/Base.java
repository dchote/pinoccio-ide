...
public class Base {
	...
	static String VERSION_NAME = "0004";
	...
	// Support multiple workspaces (for sketches and libraries)
	static String[] workspaces = {"Pinoccio", "Arduino", "Ardupilot"};
...
//		}
		...
		try {
			...
			if (mess.indexOf("ch.randelshofer.quaqua.QuaquaLookAndFeel") == -1) {
				...
				System.err.println(_("The error message follows, however Pinoccio should run fine."));
	...
	static protected void initRequirements() {
		try {
			...
			Base.showError(_("Please install JDK 1.5 or later"),
										 _("Pinoccio requires a full JDK (not just a JRE)\n" +
	...
	public Base(String[] args) throws Exception {
		...
		// define workspace
		currentWorkspace = Preferences.get("workspace");
		if (currentWorkspace == null) {
			currentWorkspace = workspaces[0];
			Preferences.set("workspace", currentWorkspace);
		}
		
		// bootstrap sketchbook stuff
		initSketchbook();
	...
	protected static void initSketchbook() {

		
		// Get the sketchbook path, and make sure it's set properly
		String sketchbookPath = Preferences.get("sketchbook.path." + currentWorkspace);
			...
			if (!sketchbookFolder.exists()) {
				Base.showWarning(_("Sketchbook folder disappeared"),
								_("The sketchbook folder no longer exists.\n" +
												"Pinoccio will switch to the default sketchbook\n" +
												"location, and create a new sketchbook folder if\n" +
												"necessary. Pinoccio will then stop talking about\n" +
		...
		if (sketchbookPath == null) {
			...
			if (portableFolder != null)
				...
				Preferences.set("sketchbook.path." + currentWorkspace, defaultFolder.getAbsolutePath());
	...
	*/
	...
	protected String createNewUntitled() throws IOException {
		...
		FileUtils.copyFile(new File(getContentFile("examples"), "Begin" + File.separator + "Begin.ino"), newbieFile);
			...
			if (Base.isMacOS()) {
				...
				String prompt =
					_("<html> " +
						...
						"<p>Closing the last open sketch will quit Pinoccio.");
				...
				if (new File(path).exists()) {
					...
					showWarning(_("Sketch Does Not Exist"),
											_("The selected sketch no longer exists.\n"
													+ "You may need to restart Pinoccio to update\n"
	...
	public void handleAbout() {
		...
		final Window window = new Window(activeEditor) {
				public void paint(Graphics g) {
					...
					g.drawString("Version: " + Base.VERSION_NAME + " Forked and maintained by Daniel Chote (http://www.chote.com)", 20, 30);
				}
			};
		...
		if (preferencesPath != null) {
			...
			try {
				...
				showError(_("Problem getting data folder"),
									_("Error getting the Pinoccio data folder."), e);
			}
		}
		...
		if (!settingsFolder.exists()) {
			if (!settingsFolder.mkdirs()) {
				showError(_("Settings issues"),
								_("Pinoccio cannot run because it could not\n" +
