...
//					System.err.println("deactivate");	 // not coming through
		...
		//TODO DAN textarea.addKeyListener(toolbar);
...
//					}
		...
		item = new JMenuItem(_("Visit Pinocc.io"));
		item.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					Base.openURL(_("http://pinocc.io/"));
				}
			});
		menu.add(item);
		...
		if (!Base.isMacOS()) {
			menu.addSeparator();
			item = new JMenuItem(_("About Pinoccio"));
			item.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						base.handleAbout();
					}
...
//			code.undo = new UndoManager();
	...
	public void handleRun(final boolean verbose) {
		internalCloseRunner();
		running = true;
		toolbar.activate(EditorToolbar.VERIFY);
	...
	class DefaultRunHandler implements Runnable {
		public void run() {
			...
			status.unprogress();
			toolbar.deactivate(EditorToolbar.VERIFY);
		}
	}
...
//		}
	...
	synchronized public void handleExport(final boolean usingProgrammer) {
		...
		toolbar.activate(EditorToolbar.UPLOAD);
		console.clear();
	...
	class DefaultExportHandler implements Runnable {
		public void run() {
			...
			uploading = false;
			//toolbar.clear();
			toolbar.deactivate(EditorToolbar.UPLOAD);

			if (success) { // TODO DAN wrap in preferences
				handleSerial();
			}
		}
	}
